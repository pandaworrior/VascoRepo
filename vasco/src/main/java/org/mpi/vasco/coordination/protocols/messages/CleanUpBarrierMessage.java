 ...
 * a permission.
 */
public class CleanUpBarrierMessage extends MessageBase{
	 ...
	 */
	public CleanUpBarrierMessage(ProxyTxnId _proxyTxnId){
		super(MessageTags.CLEANUPBARRIER, computeByteSize(_proxyTxnId));
    	...
    	public void encodeMessage(	ProxyTxnId _proxyTxnId, LockReply _lockRly){
	    	...
	    	this.config(MessageTags.CLEANUPBARRIER, computeByteSize(_proxyTxnId));
	    	int offset = getOffset();
	    	proxyTxnId.getBytes(getBytes(), offset);
	    	offset += proxyTxnId.getByteSize();
    	 ...
    	 * @return the int
    	 */
    	static int computeByteSize(ProxyTxnId _proxyTxnId){
		return _proxyTxnId.getByteSize();
	    }
    	...
    	public String toString(){
	    	String _str = "<"+getTagString()+", "+this.getProxyTxnId().toString() +">";
	    	return _str;
	    }
